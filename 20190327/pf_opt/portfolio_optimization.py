...
def main(monthly_best_file_name, training_file_name, year, month, ga_start_date, ga_end_date, test_start_date, test_end_date, index):
    ...
    if generation_number == GENERATION_SIZE:
        ...
        save.print_best_population(best_chromosome, ga_start_date, ga_end_date)
        save.save_best_generation(best_chromosome, year, month, monthly_best_file_name)
        test_fund.test_best_population(best_chromosome, test_day_list, test_stock_price_list, test_start_date, test_end_date)
...
def multiple_term(month, year, ga_start_date, ga_end_date, test_start_date, test_end_date):
    ...
    for index in range(TEST_NUM):
        main(monthly_best_file_name, training_file_name, year, month, ga_start_date, ga_end_date, test_start_date, test_end_date, index)
...
def multiple_year(year_count):
    ...
    for MONTH in range(MONTHS):
        ...
        monthly_process = threading.Thread(target=multiple_term, args=(month_count, year_count, ga_start_date, ga_end_date, test_start_date, test_end_date))
...
if __name__ == '__main__':
    ...
    for year_count in range(YEAR, 2019, 1):
        annual_process = threading.Thread(target=multiple_year, args=(year_count,))
